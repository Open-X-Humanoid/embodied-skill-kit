...
# -*- coding: utf-8 -*-
"""
skill02_bottle_grasp · 姿态小工具（纯 numpy，无额外依赖）
...
def rpy_to_quat(roll, pitch, yaw):
    """(roll, pitch, yaw) rad（ZYX，与 quat_to_rpy 同约定）→ 四元数 [x,y,z,w]。
    人工标定固定手部姿态(GRASP_ORIENT)时，给 RPY 比手搓四元数直观，标定完用这个转一次即可。"""
...
def quat_from_zaxis(z_axis, up=(0.0, 0.0, 1.0), spin=0.0):
    ...
    if np.linalg.norm(x) < 1e-6:    # 法线近竖直——用世界 x 兜底
