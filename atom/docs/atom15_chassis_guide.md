# Atom 15 · Chassis · Timed Motion (Slamtec REST)

**English** | [简体中文](atom15_chassis_guide_zh-CN.md)
...
|---|---|
| `atom/demos/atom15_chassis_slamware.py` (runs on the real robot), `atom/demos/atom15_chassis_slamware_robust.py` (production) | in `atom/assets/videos/`, same name as the code — watch 30s first |
...
python3 atom/demos/atom15_chassis_slamware.py
...
`atom15_chassis_slamware.py` is **5 modules**. Note the chassis uses **REST (`requests`)**, a different interface family from the ROS2 topics of the other atoms.
...
- **Production hardening** (see `atom/demos/atom15_chassis_slamware_robust.py`): HTTP retry, signal handling via a flag rather than side effects inside the handler, **before/after pose self-check that auto-warns on "reported success but didn't move,"** and soft stop.
