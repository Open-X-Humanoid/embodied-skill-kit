...
- ★**跑前 source 的是 XARM，不是 ros2ws**：`source /home/ubuntu/XARM/install/setup.bash`（`moveit_msgs`、`tianyi2_bringup` 都在这）。
...
> **强烈建议先 sim**：手臂力矩大、范围大，先在仿真里把规划/执行跑通、确认无误，再切 `real`。ROS 基础环境见《前置 · 环境配置》(`atom/docs/environment_setup_zh-CN.md`)。
...
**XARM 的"使能"不是电机上电，而是"XARM 要不要向 body_control 发指令"。** 不使能，MoveIt 规划/执行都报成功、控制器也 OK，但 XARM 根本不发 `/arm/cmd_pos` → 物理臂纹丝不动。demo 里 `enable_arm()` 调 `/EAIHardware/set_arm_enable` 使能；手动等价：
...
| 模块 | 代码锚点 | 职责 | 换右臂/双臂要改？ |
|---|---|---|---|
| 使能 | `enable_arm()` → `/EAIHardware/set_arm_enable` | real 模式开"发指令的门" | 不变（`set_all_enable` 可一次全使能） |
...
> 底层殊途同归：两者最终都落到 `/arm/cmd_*` → body_control → 电机。差别只在"人肉直发" vs "MoveIt 规划后由 XARM 发"。也正因抢同一个 `/arm/cmd_pos`，**atom04 和 XARM 使能互斥**（跑着 atom04 时 XARM 使能会失败）。
...
| 使能失败 / 自动关使能 | 别的程序在发 `/arm/cmd_pos`（如残留 atom04） | `bash scripts/stop_all.sh` 清场后重来 |
| 真机"看门狗超时" | body_control 没起 / 没收到 `/arm/status` | 先起 body_control，再起 XARM |

自查两条：`ros2 service call /EAIHardware/debug ...`（看 `arm_enable=1`）、`ros2 control list_controllers | grep -i moveit`（看控制器 active）。
