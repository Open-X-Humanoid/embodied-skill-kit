# 原15 · 底盘（Chassis）· 限时移动（思岚 REST）

[English](atom15_chassis_guide.md) | **简体中文**

**一句话**：用思岚底盘的 REST 动作接口（`MoveByAction`，**限时自停**）让底盘前进/后退/转向，读 `/localization/pose` 看位姿（状态）。

| 配套代码 | 演示视频 |
|---|---|
| `atom/demos/atom15_chassis_slamware.py`（真机可运行版）、`atom/demos/atom15_chassis_slamware_robust.py`（生产版） | `atom/assets/videos/` 下与配套代码同名，建议先看 30 秒 |
...
`atom15_chassis_slamware.py` 全文 = **5 个模块**。注意底盘走 **REST（`requests`）**，和其它原子的 ROS2 话题是不同的接口家族。
...
- **生产级加固**（见 `atom/demos/atom15_chassis_slamware_robust.py`）：HTTP 失败重试、信号用标志位而非在处理器里执行副作用、**运动前后位姿自检并自动报警「报成功却没动」**、软急停。
