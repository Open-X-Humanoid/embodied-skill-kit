# 原8 · 手臂（Arm）· XARM QP 末端（笛卡尔）控制

[English](atom08_arm_qp_endpose_guide.md) | **简体中文**

**一句话**：给 XARM 的 **QP 末端控制器**（`endpose_single_arm_qp_L_controller`）一个"末端位姿"目标，它做**局部响应式跟踪**把 TCP 送过去，自带防碰撞。和 atom06 的区别：**atom06 是 MoveIt"一次规划整条轨迹"，atom08 是 QP"在线小步跟踪"**——目标离当前太远会被直接拒绝（误差限），只能小幅增量喂。

| 配套 | 路径 |
|---|---|
| 代码（简洁版） | `atom/demos/atom08_arm_qp_endpose.py` |
| 演示视频 | 与配套代码同名，见 `atom/assets/videos/` |

> 建议先读 atom06（末端控制的坐标系/TF 读起点）和 atom07（QP 的使能/切控制器/互斥）；本文只展开 atom08 独有的部分。

## 1. 速览

### 1.1 跑起来（同 atom07——QP 不需要 MoveIt 组件）
...
> 本控制器**自带防碰撞**（身体 2 球 + 手臂 7 球模型），比 atom07 的关节控制器安全一档；但仍需臂周围无人无物、急停在手。
...
> 对照 atom06：MoveIt 给多远都行（它整条规划）；QP 给远了直接拒。这不是缺陷，是"响应式跟踪器"的设计——它假设上游（如视觉）在高频喂邻近目标。

## 2. 核心操作

### 2.1 使能 + 切控制器（同 atom07,不重复）

`enable_arm()` 多候选探测 + `_scan_arm_controllers()` 自动停占用本臂的控制器 + STRICT 切换。

### 2.2 发位姿目标

`move_to_pose()` 填 `ArmTargetPose`（from/to/target/offset）→ 发 action → 阻塞等 `result.success`。**本 action 是等到位才返回的**（按 `dis_threshold`/`ori_threshold`/`step_threshold` 停止），与 atom07 的"返回≠到位"不同——末端版不需要额外的 wait_reached。
...
| 使能相关报错 | 与 atom05/07 同源 | 见《原5》排错表；`sudo systemctl stop teleop_robot` |
