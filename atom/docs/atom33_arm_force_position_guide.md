# Atom 33 · Arm · Force–Position Hybrid Control

**English** | [简体中文](atom33_arm_force_position_guide_zh-CN.md)

**In one line**: publish a `CmdMotorCtrl` to `/arm/cmd_ctrl` to overlay velocity/torque feedforward on a position target and tune joint stiffness with `kp`/`kd`; return to the start with `/arm/cmd_pos` when done. Read atom04 (position mode) first.

| Companion code |
|---|
| `atom/demos/atom33_arm_force_position_ros2.py` (native ROS2), `atom/demos/atom33_arm_force_position_ros2_robust.py` (production) |
...
python3 atom/demos/atom33_arm_force_position_ros2.py
```
...
The simple version **does no validation** (target out of range, `kp`/`kd` out of legal range, publisher readiness — all published as-is). The production file `atom33_arm_force_position_ros2_robust.py` adds those checks — see §6.
...
`atom33_arm_force_position_ros2.py` follows the same lean skeleton as atom04 and peers, plus a small helper that keeps spinning during the hold.
...
| 4 | Wait for angle | `wait_for_status` | same as atom04 — spin until joint angle arrives |
...
**Production hardening** (`atom33_arm_force_position_ros2_robust.py`): same pattern as `atom04_arm_ros2_robust.py` / `atom01_head_ros2_robust.py`, plus two hybrid-mode-specific items:
