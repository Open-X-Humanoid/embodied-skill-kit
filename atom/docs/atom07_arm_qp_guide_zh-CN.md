# 原7 · 手臂（Arm）· XARM QP 关节空间控制

[English](atom07_arm_qp_guide.md) | **简体中文**

**一句话**：把 7 个目标关节角交给 XARM 的 **QP 关节控制器**（`jointspace_arm_L_controller`），它在线做速度/加速度/jerk 平滑与限位检查后执行。和 atom05 的区别只有一处：**atom05 是 MoveIt"一次规划整条轨迹再执行"，atom07 是 QP"响应式在线跟踪"**——收到新目标立即平滑跟过去，适合目标不断变化的场景（如视觉伺服）。

| 配套 | 路径 |
|---|---|
| 代码（简洁版） | `atom/demos/atom07_arm_qp.py` |
| 演示视频 | 与配套代码同名，见 `atom/assets/videos/` |

> 建议先读 atom05：使能、切控制器这两件事各手臂原子共用；本文只展开 QP 特有的部分。

## 1. 速览

### 1.1 跑起来（比 atom05 少一步——QP 不需要 MoveIt 组件）
...
> ⚠ **本控制器无防碰撞**（XARM 手册明示；防自碰版是 `jointspace_arm_qpik_L_controller`，但它不支持 action）。臂周围无人无物、急停在手。ROS 基础环境见《前置 · 环境配置》(`atom/docs/environment_setup_zh-CN.md`)。
...
### 1.3 关节限位（同 atom05，此处不重复）

7 关节限位表见《原5 · 手臂 · XARM MoveIt 关节运动》(`atom/docs/atom05_arm_moveit_guide_zh-CN.md`) 1.3 节。QP 控制器**自带限位检查**：超上限报 `600101`、超下限报 `600102`，指令被拒、臂不动。

## 2. 核心操作（含一个 QP 特有的实测坑）

### 2.1 使能 + 切控制器（同 atom05/06 套路）
...
> 对照 atom05：MoveIt 的 action result 是**执行完才返回**（轨迹控制器确认到位），所以 atom05 不需要这一步。这是"离线规划"和"在线跟踪"在使用手感上最重要的差别。
...
| 现象 | 原因 | 处理 |
|---|---|---|
| `600101` / `600102` | 目标超上/下限位 | 对照 atom05 限位表；从当前角小步增量 |
...
| 使能相关报错 | 与 atom05 完全同源：服务改名、teleop 占话题、重复使能噪音 | 见《原5》排错表逐条对号；`sudo systemctl stop teleop_robot` |
...
- **调速**：`vel_limits`（7 关节速度上限 rad/s）是最直接的总阀门，命令行 `ros2 param set` 即时生效；只读参考 `acc_limits`/`jerk_limits`。⚠ 手册明示 vel_limits 不应超过物理机构执行能力——**只往小调是安全方向**。代码级调法见 atom08 的 `set_vel_limits()`（同一套参数服务，可直接搬过来）。
...
- **末端版**：想给"末端位姿"而不是关节角，见《原8 · 手臂 · XARM QP 末端控制》(`atom/docs/atom08_arm_qp_endpose_guide_zh-CN.md`)。
