# 语音 · 输出侧（Voice Output）· 朗读与放音频（atom26 / atom27）

[English](atom26-27_voice_output_guide.md) | **简体中文**

**一句话**：让机器人"出声"。atom26 把一段文字读出来（TTS），atom27 播放音频文件 / 网络 URL 并支持暂停·恢复·停止。两者都只是**调用 lyre 的 `/audio_play/*` 服务**，起一个 lyre 就能跑，不碰电机、不需麦克风。

| 配套 | 路径 |
|---|---|
| 代码（朗读） | `atom/demos/atom26_voice_tts_ros2.py` |
| 代码（放音频） | `atom/demos/atom27_voice_play_ros2.py` |
...
> lyre 有 `play/asr/audio/chat` 四种互斥模式，**chat（出厂默认）覆盖全部语音功能**，统一用它即可、无需切模式。ROS 基础环境见《前置 · 环境配置》(`atom/docs/environment_setup_zh-CN.md`)。
...
python3 atom/demos/atom26_voice_tts_ros2.py  # 机器人读出代码里的 TEXT
python3 atom/demos/atom27_voice_play_ros2.py # 交互菜单：1 播文件 / 2 播URL / 3 暂停 / 4 恢复 / 5 停止 / q 退出
```
...
字段含义：`sid` = 唯一播放流 ID（demo 用 `atom26_<随机>`）；`seq`/`last` = 分包序号/末包标志，单次播放填 `0`/`True`；`force` = `True` 打断当前播放 / `False` 排队；`token`/`output` = 系统内部字段，应用层**留空**。响应 `code`：`0` 成功 / `1` 参数非法 / `-1` 内部失败。
...
**举一反三**：想调 lyre 任何别的服务，都是这四步——`create_client` → 填 `Request` → `call_async`+`spin_until_future_complete` → 查 `code`。atom27 里五个服务共用一个 `_call()` 就是这个道理。

## 4. 改一改，看变化

- atom26：改 `TEXT`；把 `force=True` 改 `False`，在有音频播放时调用会**排队**而非打断。
- atom27：改 `FILE_PATH` 指向别的文件；连续选 `1` 再选 `3/4/5` 体会暂停/恢复/停止；先播一段再选 `1` 播另一段（`force=True`）看打断效果。
