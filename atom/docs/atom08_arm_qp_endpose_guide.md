# Atom 8 · Arm · XARM QP Cartesian (End-Effector) Control

**English** | [简体中文](atom08_arm_qp_endpose_guide_zh-CN.md)

**In one line**: give XARM's **QP end-effector controller** (`endpose_single_arm_qp_L_controller`) an end-effector *pose* goal and it performs **local responsive tracking** to bring the TCP there, with built-in collision avoidance. Difference from atom06: **atom06 is MoveIt "plan a whole trajectory once"; atom08 is QP "online small-step tracking"** — a goal too far from the current pose is rejected outright (error bounds), so you can only feed small increments.

| Companion | Path |
|---|---|
| Code (simple) | `atom/demos/atom08_arm_qp_endpose.py` |
| Demo video | same name as the code, under `atom/assets/videos/` |

> Read atom06 first (frames / reading the start pose via TF) and atom07 (QP enable / controller switching / mutual exclusion); this guide only expands what is unique to atom08.

## 1. Overview

### 1.1 Run it (same as atom07 — no MoveIt component needed)
...
python3 atom/demos/atom08_arm_qp_endpose.py
```
...
> This controller has **built-in collision avoidance** (2 body spheres + 7 arm spheres) — one notch safer than atom07's joint controller; still keep people and objects away, E-stop in hand.
...
|---|---|---|
| `from_frame` | `base` | reference frame (XARM manual's recommendation for TianYi; `base` coincides with the root `base_footprint` at zero offset, see atom06 guide §1.3) |
...
### 1.4 ★Error bounds: a far goal is rejected outright (the biggest difference from atom06)
...
> Contrast with atom06: MoveIt accepts any distance (it plans the whole path); QP rejects far goals. Not a defect — a responsive tracker assumes an upstream source (e.g. vision) feeding nearby goals at high frequency.

## 2. Core operations

### 2.1 Enable + switch controller (same as atom07, not repeated)

`enable_arm()` probes candidates + `_scan_arm_controllers()` auto-stops whoever claims the arm + STRICT switch.

### 2.2 Send the pose goal

`move_to_pose()` fills `ArmTargetPose` (from/to/target/offset) → sends the action → blocks for `result.success`. **This action returns only on arrival** (it stops per `dis_threshold`/`ori_threshold`/`step_threshold`) — unlike atom07's "return ≠ arrival", so no extra wait_reached is needed here.
...
## 4. Difference from atom06 (MoveIt Cartesian)

| | atom06 (MoveIt) | atom08 (QP) |
|---|---|---|
...
| Enable-related errors | same roots as atom05/07 | see *Atom 5*'s troubleshooting table; `sudo systemctl stop teleop_robot` |
