# Voice · Output Side · TTS & Playback (atom26 / atom27)

**English** | [简体中文](atom26-27_voice_output_guide_zh-CN.md)

**In one line**: make the robot *speak*. atom26 reads a piece of text aloud (TTS); atom27 plays an audio file / URL with pause·resume·stop. Both just **call lyre's `/audio_play/*` services** — one running lyre is enough, no motors, no microphone.

| Companion | Path |
|---|---|
| Code (TTS) | `atom/demos/atom26_voice_tts_ros2.py` |
| Code (playback) | `atom/demos/atom27_voice_play_ros2.py` |
...
> lyre has four mutually-exclusive modes `play/asr/audio/chat`; **chat (factory default) covers all voice features**, so just use it — no mode switching. For the base ROS environment see *Prerequisite · Environment Setup* (`atom/docs/environment_setup.md`).
...
python3 atom/demos/atom26_voice_tts_ros2.py  # robot reads the TEXT in the code
python3 atom/demos/atom27_voice_play_ros2.py # menu: 1 file / 2 URL / 3 pause / 4 resume / 5 stop / q quit
```
...
Field meaning: `sid` = unique playback-stream ID (the demo uses `atom26_<random>`); `seq`/`last` = packet index / last-packet flag, use `0`/`True` for a one-shot; `force` = `True` interrupts current playback / `False` queues; `token`/`output` = internal system fields, leave **empty**. Response `code`: `0` success / `1` bad args / `-1` internal failure.
...
**Generalize**: to call any other lyre service, it's the same four steps — `create_client` → fill `Request` → `call_async`+`spin_until_future_complete` → check `code`. That's why atom27's five services all share one `_call()`.

## 4. Tweak and observe

- atom26: change `TEXT`; set `force=False` and call it while audio is playing to **queue** instead of interrupt.
- atom27: point `FILE_PATH` at another file; pick `1` then `3/4/5` to feel pause/resume/stop; play one clip then `1` another (`force=True`) to see the interrupt.
