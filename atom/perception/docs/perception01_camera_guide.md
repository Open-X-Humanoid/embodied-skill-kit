# Perception 1 · Camera · Read Image / Depth / Intrinsics (Orbbec)

**English** | [简体中文](perception01_camera_guide_zh-CN.md)
...
|---|---|
| `atom/perception/perception01_camera_orbbec.py` (native ROS2, RGB), `atom/perception/perception01_camera_orbbec_robust.py` (production, RGB+depth+intrinsics) | in `atom/perception/assets/videos/`, same name as the code |
...
python3 atom/perception/perception01_camera_orbbec.py
```

Expected: `收到一帧 RGB 图: 1280x720` → saved to `atom/perception/assets/camera_captures/perception01_rgb.jpg`.
...
cv2.imwrite("perception01_rgb.jpg", node.rgb)      # save it to see the result
```
...
`perception01_camera_orbbec.py` (plain version) is **4 modules**. The opposite of a motion atom — it **only subscribes, never publishes** (perception).
...
- **Module 1 — config constants**: the RGB topic name, output dir (`atom/perception/assets/camera_captures`), first-frame timeout.
