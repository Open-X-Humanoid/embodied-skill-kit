# 感知1 · 相机（Camera）· 读取图像 / 深度 / 内参（Orbbec）

[English](perception01_camera_guide.md) | **简体中文**

**一句话**：订阅 Orbbec 相机的 ROS2 话题（RGB `/camera/color/image_raw`、深度、内参），用 `cv_bridge` 转成 numpy 图像并保存。这是**感知类原子——只读订阅，不控制任何电机、机器人不会动**。

| 配套代码 | 演示视频 |
|---|---|
| `atom/perception/perception01_camera_orbbec.py`（ROS2 原生版，取 RGB）、`atom/perception/perception01_camera_orbbec_robust.py`（生产版，取 RGB+深度+内参） | `atom/perception/assets/videos/` 下与配套代码同名 |
...
预期现象：`收到一帧 RGB 图: 1280x720` → 存到 `atom/perception/assets/camera_captures/perception01_rgb.jpg`。
...
cv2.imwrite("perception01_rgb.jpg", node.rgb)      # 存下来看效果
```
...
`perception01_camera_orbbec.py`（简洁版）全文 = **4 个模块**。和运动原子相反——它是**只订阅、不发布**（感知）。
...
- **模块 1 配置常量**：RGB 话题名、输出目录（`atom/perception/assets/camera_captures`）、等首帧超时。
...
- **生产版（RGB+深度+内参三件套）**：`atom/perception/perception01_camera_orbbec_robust.py`——三路订阅、`wait_for_frames` 收齐再处理、打印内参 `K` 与深度统计、按深度图自身尺寸取中心深度、存 RGB.jpg + 16 位深度.png + 伪彩色深度.jpg。回调带 try/except、非负 spin。
