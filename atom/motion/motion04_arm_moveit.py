...
# -*- coding: utf-8 -*-
"""
原子示例 · 运控4 · 手臂（Arm）· XARM MoveIt 关节运动
...
简洁版 vs robust
  · 本文件=简洁版：看懂原理即可。切控制器前自动停掉占用本臂的其它 active 控制器（各族互斥，
    如刚跑过 QP demo 残留的），STRICT 切换、失败如实报错。
  · motion04_arm_moveit_robust.py=生产版：在此之上再加 MoveGroup 发送/执行的严格超时与 None 校验。
...
# 两个都是 std_srvs/SetBool，运行时逐个探测、调失败自动换下一个。新名在前，探测更快。
...
def main():
    ...
    node = ArmMoveItDemo()
    log = node.get_logger()
    log.info(f"运控4 · 手臂 MoveIt 关节运动 | 规划组={GROUP} vel={VEL_SCALE} acc={ACC_SCALE} 演示增量={DEMO_DELTA}")
