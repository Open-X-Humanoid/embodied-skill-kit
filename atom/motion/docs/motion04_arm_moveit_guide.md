# Motion 4 · Arm · XARM MoveIt Joint Motion

**English** | [简体中文](motion04_arm_moveit_guide_zh-CN.md)

**In one line**: let **MoveIt** plan and smoothly move a 7-DOF arm to a target joint configuration — instead of publishing raw angles yourself (that's motion03), you "tell it where to go, and it computes a collision-free trajectory and executes it." This atom uses the **standard MoveIt2 `/move_action`** (no project wrapper); execution goes through XARM's `moveit_*_arm_controller`.

| Companion | Path |
|---|---|
| Code (simple) | `atom/motion/motion04_arm_moveit.py` |
| Code (robust) | `atom/motion/motion04_arm_moveit_robust.py` (auto-avoids controllers claiming this arm + stricter checks) |
...
> Relationship to motion03 is in §4: motion03 = "you publish angles by hand", motion04 = "MoveIt plans, XARM publishes" — same bottom layer.

## 1. Overview

### 1.1 Run it (heavier prerequisites than motion03 — follow the order)
...
python3 atom/motion/motion04_arm_moveit.py
```
...
> Enabling **checks for conflicts**: if another app publishes `/arm/cmd_pos` (e.g. a leftover motion03), enable fails or gets force-disabled — clear first with `bash scripts/stop_all.sh`.
...
## 4. Difference from motion03

| | motion03 (raw) | motion04 (MoveIt) |
|---|---|---|
...
> Same bottom layer: both end at `/arm/cmd_*` → body_control → motors. The only difference is "publish by hand" vs "MoveIt plans, XARM publishes". Because they contend for the same `/arm/cmd_pos`, **motion03 and XARM enable are mutually exclusive in both directions**: enable fails while motion03 is running; conversely, **while XARM is enabled (which it is after running this atom) it streams commands at 250Hz, so motion03's direct commands are instantly overridden → the arm "silently doesn't move"**. One arm, one command source at a time — the enable switch is the hand-over of the steering wheel. Both directions are handled automatically: this atom auto-enables; motion03 auto-disables XARM enable when it detects it — switching back and forth needs no manual commands.
...
| enable fails / auto-disables | another app publishes `/arm/cmd_pos` (e.g. leftover motion03, or the teleop `teleop_dispatcher`) | `bash scripts/stop_all.sh`, then retry; `ros2 topic info /arm/cmd_pos -v` to list publishers (should be just 1) |
...
- **Cartesian (end-effector) control**: this atom is joint-space MoveIt; to command an end-effector pose (instead of joint angles), see **motion05 (MoveIt Cartesian control)**.
