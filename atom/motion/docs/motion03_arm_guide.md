# Motion 3 · Arm · Single-Joint Position Control

**English** | [简体中文](motion03_arm_guide_zh-CN.md)
...
|---|---|
| `atom/motion/motion03_arm_ros2.py` (native ROS2), `atom/motion/motion03_arm_ros2_robust.py` (production) | in `atom/motion/assets/videos/`, same name as the code |
...
python3 atom/motion/motion03_arm_ros2.py
```
...
> Compared with the head (motion01): the head sends 3 joints at once, the arm sends 1 — the `cmds` list length differs, the pattern is the same.
...
`motion03_arm_ros2.py` is **6 modules**. It's the same "position control" skeleton as the head (motion01); the only difference is the arm controls **one joint at a time**.
...
# shape the send method to the part: the arm sends one joint at a time (move_joint); the head sends 3 (move_to, see motion01)
...
- **Production hardening**: wait-for-subscriber-ready, return `None` and refuse on status timeout, soft-limit + step-amplitude dual checks, `spin_once` (non-negative timeout) instead of `time.sleep` — see `atom/motion/motion03_arm_ros2_robust.py`.
