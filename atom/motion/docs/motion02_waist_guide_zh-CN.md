...
[English](motion02_waist_guide.md) | **简体中文**
...
| 配套代码 | 演示视频 |
|---|---|
| `atom/motion/motion02_waist_ros2.py`（ROS2 原生版）、`atom/motion/motion02_waist_ros2_robust.py`（生产版） | `atom/motion/assets/videos/` 下与配套代码同名 |
...
⚠ **pitch(32) 与下肢升降耦合**：单独乱动会改变整机重心、有**倾覆风险**——本 demo 只把 pitch **保持**在当前测量值，不主动动它。要动 pitch 请走腿部 motion10（与升降协调）。
...
`motion02_waist_ros2.py` 全文 = **6 个模块**。和头/臂是同一套「发指令 + 读状态」骨架，特殊点在**一次发两个关节：yaw 动、pitch 保持**。
...
| `SPEED` 调小 | 转得更慢 |
| ~~主动改 pitch~~ | ⚠ 别动 pitch，倾覆风险；要升降走 motion10 腿 |
...
- **生产级加固（含电压/故障自检）**：`atom/motion/motion02_waist_ros2_robust.py` 的 `_check_motor_errors` 会在**每次运动前**读 `/waist/status` 的 `error` 字段，发现非 0 错误码（如 **`12832` 欠压**）就**拒绝运动并打印**——提前发现电压/电机故障，不带病运动。此外还有软限位校验、等订阅就绪、`spin_once` 非负 timeout。
- **要动 pitch（前倾 / 升降）**：走腿部 **motion10**，与下肢协调升降，别单独动腰 pitch（倾覆风险）。
