# 运控10 · 下肢（Leg，腰+腿协调）· 协调升降与带力保持

[English](motion10_leg_guide.md) | **简体中文**
...
| 配套代码 | 演示视频 |
|---|---|
| `atom/motion/motion10_leg_ros2.py`（ROS2 原生版）、`atom/motion/motion10_leg_ros2_robust.py`（生产版） | `atom/motion/assets/videos/` 下与配套代码同名，**尤其建议先看**别人跑对是什么样 |

## 1. 速览（点进来先看这块）

### 1.1 跑起来

前提：已按《前置 · 环境配置》（`docs/environment_setup_zh-CN.md`）起好 body_control。**物理准备**：底座停稳、升降空间内无人无物、物理急停在手。（轮式底座已提供横向稳定，一般无需吊挂；仅在试**大幅度**升降或配合手臂大幅伸展时才需额外防护。）
...
`motion10_leg_ros2.py`（简洁版，已瘦身）全文 = **7 个模块**。它比其它原子复杂，因为要**跨两个话题控 4 个电机 + 插值 + 带力保持**。
...
- **生产级加固**（见 `atom/motion/motion10_leg_ros2_robust.py`）：**电机错误逐步自检**（发现即带力保持退出）、单关节**位移硬上限** `SAFETY_CAP`、线程锁保护共享状态、**标志位式信号处理**（不在信号处理器里 `shutdown`，避免竞态）、`destroy_node` 收尾。承重原子真上业务用这版。
