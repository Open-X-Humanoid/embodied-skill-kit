# Motion 5 · Arm · XARM MoveIt Cartesian (End-Effector) Control

**English** | [简体中文](motion05_arm_moveit_endpose_guide_zh-CN.md)

**In one line**: give **MoveIt** an end-effector *pose* goal — where the hand should go in space (x/y/z) and how it should be oriented (quaternion) — and it solves IK, plans a collision-free trajectory, and executes it. Exactly one thing differs from motion04: **motion04 gives joint angles, motion05 gives an end-effector pose**. Everything else (enable, controller switch, `/move_action`) is identical.

| Companion | Path |
|---|---|
| Code (simple) | `atom/motion/motion05_arm_moveit_endpose.py` |
| Demo video | same name as the code, under `atom/motion/assets/videos/` |

> No `_robust` version yet. For controller conflicts / `CONTROL_FAILED(-4)`, borrow the approach in `atom/motion/motion04_arm_moveit_robust.py` (strict switching + stepping aside from controllers claiming this arm) — it ports over directly.

> Read motion04 first: enable, controller switch, and `/move_action` are shared by both atoms and aren't re-explained here.

## 1. Overview

### 1.1 Run it (same prerequisites as motion04)
...
python3 atom/motion/motion05_arm_moveit_endpose.py
```

> ⚠ **High-frequency pitfall: instant `error_code=99999`** — MoveIt's joint limits are **tighter** than the URDF (e.g. `shoulder_yaw_l_joint`: MoveIt=±1.5 vs URDF=±2.96), while QP/teleop follow the wider URDF limits. **After a QP atom (motion06/08) or teleop, the arm often parks outside MoveIt's bounds → this atom gets instantly rejected with 99999** (signature: millisecond failure, QP atoms fine). Check: `tmux capture-pane -t xarm.1 -p -J -S -400 | grep -i 'outside bounds'` names the joint; fix: move it back via QP (see the 99999 row in Motion 4's troubleshooting table).
...
### 1.2 Interfaces (one more than motion04: TF)
...
| **`/tf`, `/tf_static`** | `tf2_msgs/TFMessage` (Topic) | **read the current TCP pose as the motion start** (motion04 uses `/joint_states`; here it's TF) |

> Why the start point switches to TF: motion04's start is "7 joint angles", which `/joint_states` hands you directly. motion05's start is "where the end-effector is in space" — turning joint angles into an end-effector pose requires **forward kinematics**, and the TF tree has already been computed and published live by `robot_state_publisher`. Just query it instead of doing the math.

### 1.3 ★The two frames (motion05's #1 pitfall)
...
Joint limits still apply — they just **stop you differently**. In motion04 you give joint angles directly and an out-of-range one is rejected. In motion05 you give a pose, **MoveIt solves IK first**, and if the resulting joint angles violate limits or the point simply can't be reached, it returns `NO_IK_SOLUTION(-31)` — **not a "limit error" but "no solution"**.

- The per-joint limit table lives in *Motion 4 · Arm · XARM MoveIt Joint Motion* (`atom/motion/docs/motion04_arm_moveit_guide.md`) §1.3 and isn't duplicated here.
...
## 2. Four must-dos (one more than motion04: reading TF)

### 2.1 Enable the arm (the #1 real-mode pitfall; same as motion04)
...
### 2.2 Switch to the MoveIt controller (same as motion04)
...
### 2.3 ★Read the current end-effector pose (new in motion05)
...
### 2.4 Send the pose goal (the real difference from motion04)

motion04 sends 7 `JointConstraint`s; motion05 sends **one position constraint + one orientation constraint**, both on `EE_LINK`:
...
## 4. Difference from motion04

| | motion04 (joint space) | motion05 (Cartesian) |
|---|---|---|
...
**Which one when**: aligning the hand to an object (grabbing a box, placing a part) → motion05, because you know where the object is, not how far each joint should turn. Fixed pose changes (return home, raise a hand) → motion04, because the joint angles are known and you skip the IK step entirely.
...
| enable errors / spawners stuck at `waiting for list_controllers` / plan succeeds but arm static | same root causes as motion04: renamed enable service, redundant-enable noise, teleop occupying the topic, missing library after system upgrade (pinocchio) | see the troubleshooting table in *Motion 4 · Arm MoveIt joint motion* (`atom/motion/docs/motion04_arm_moveit_guide.md`) |
...
| `error_code=-31` NO_IK_SOLUTION | target unreachable / IK solution violates joint limits | shrink the translation; move the arm near the target first; check motion04's limit table |
