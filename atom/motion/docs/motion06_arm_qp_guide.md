# Motion 6 · Arm · XARM QP Joint-Space Control

**English** | [简体中文](motion06_arm_qp_guide_zh-CN.md)

**In one line**: hand 7 target joint angles to XARM's **QP joint controller** (`jointspace_arm_L_controller`), which smooths velocity/acceleration/jerk online, checks limits, and executes. Exactly one thing differs from motion04: **motion04 is MoveIt "plan a whole trajectory offline, then execute"; motion06 is QP "responsive online tracking"** — each new goal is smoothly tracked the moment it arrives, suited to continuously changing targets (e.g. visual servoing).

| Companion | Path |
|---|---|
| Code (simple) | `atom/motion/motion06_arm_qp.py` |
| Demo video | same name as the code, under `atom/motion/assets/videos/` |

> Read motion04 first: enable and controller switching are shared by all arm atoms; this guide only expands what is QP-specific.

## 1. Overview

### 1.1 Run it (one step fewer than motion04 — QP needs no MoveIt component)
...
### 1.3 Joint limits (same as motion04, not duplicated)

The 7-joint limit table is in *Motion 4 · Arm · XARM MoveIt Joint Motion* (`atom/motion/docs/motion04_arm_moveit_guide.md`) §1.3. The QP controller **checks limits itself**: above the upper limit → `600101`, below the lower → `600102`; the command is rejected and the arm stays put.

## 2. Core operations (including one QP-specific pitfall found on the real robot)

### 2.1 Enable + switch controller (same pattern as motion04/06)
...
> Contrast with motion04: MoveIt's action result **returns only after execution finishes** (the trajectory controller confirms arrival), so motion04 needs no such step. This is the most important hands-on difference between "offline planning" and "online tracking".
...
## 4. Difference from motion04 (MoveIt joint)
...
| Symptom | Cause | Fix |
|---|---|---|
| `600101` / `600102` | target above/below joint limit | check motion04's limit table; step incrementally from the current angle |
...
| Enable-related errors | same roots as motion04: renamed service, teleop occupying the topic, repeat-enable noise | see *Motion 4*'s troubleshooting table; `sudo systemctl stop teleop_robot` |
...
- **End-effector version**: to command an end-effector pose instead of joint angles, see *Motion 7 · Arm · XARM QP Cartesian Control* (`atom/motion/docs/motion07_arm_qp_endpose_guide.md`).
