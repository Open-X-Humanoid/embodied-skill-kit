# 运控1 · 头部（Head）· 位置控制与读角度

[English](motion01_head_guide.md) | **简体中文**

**一句话**：向话题 `/head/cmd_pos` 发一条 `CmdSetMotorPosition` 让头部 3 个电机转到指定角度，订阅 `/head/status` 读回当前角度。

| 配套代码 | 演示视频 |
|---|---|
| `atom/motion/motion01_head_ros2.py`（ROS2 原生版） | `atom/motion/assets/videos/` 下与配套代码同名 |
...
`motion01_head_ros2.py` 全文 = **5 个模块**。任何一个「电机位置控制」部位（臂/腰/腿）都是同样 5 块，**换部位只改配置、逻辑不动**。
...
- **生产级加固**：状态等待就绪、限位校验拒发、用 `spin_once`（非负 timeout）替代 `time.sleep`——见 `atom/motion/motion01_head_ros2_robust.py`。
