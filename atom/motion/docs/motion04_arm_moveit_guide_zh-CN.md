# 运控4 · 手臂（Arm）· XARM MoveIt 关节运动

[English](motion04_arm_moveit_guide.md) | **简体中文**

**一句话**：让 **MoveIt** 帮你规划，把一条 7 自由度手臂平滑运动到指定关节角——不是自己硬发角度（那是 motion03），而是"告诉它去哪，它自己算一条无碰撞轨迹再执行"。本原子用**标准 MoveIt2 的 `/move_action`**（不依赖工程封装），执行经 XARM 的 `moveit_*_arm_controller`。

| 配套 | 路径 |
|---|---|
| 代码（简洁版） | `atom/motion/motion04_arm_moveit.py` |
| 代码（生产版 robust） | `atom/motion/motion04_arm_moveit_robust.py`（切控制器前自动避让占用本臂的控制器 + 严格超时校验） |
| 演示视频 | 与配套代码同名，见 `atom/motion/assets/videos/` |

> 简洁版看懂原理（假设 XARM 刚起、本臂控制器无冲突）；真机遇"控制器冲突 / CONTROL_FAILED(-4)"用 robust 版。

> 和 motion03 的关系见第 4 节：motion03 是"你亲手发角度"，motion04 是"MoveIt 规划后由 XARM 发"，底层殊途同归。

## 1. 速览

### 1.1 跑起来（前提比 motion03 重，务必按顺序）
...
> **仿真模式**（`bash scripts/start_xarm.sh sim`，带 RViz、不接真机/不需 body_control）：无真机时可用它先看规划/执行效果。⚠ 本项目未在 sim 下实测，命令理论可用、以你机器为准；已验证的是 real。ROS 基础环境见《前置 · 环境配置》(`docs/environment_setup_zh-CN.md`)。
...
> 使能会**检查冲突**：若有别的程序在发 `/arm/cmd_pos`（比如残留的 motion03），使能会失败甚至被强制关闭——先 `bash scripts/stop_all.sh` 清场。
...
> 底层殊途同归：两者最终都落到 `/arm/cmd_*` → body_control → 电机。差别只在"人肉直发" vs "MoveIt 规划后由 XARM 发"。也正因抢同一个 `/arm/cmd_pos`，**motion03 和 XARM 使能双向互斥**：跑着 motion03 时 XARM 使能会失败；反过来 **XARM 使能中（跑过本原子后即如此）它以 250Hz 持续发流，motion03 的直发被瞬间覆盖 → 臂"看似不动且无报错"**。一条臂同一时刻只能有一个指令源，使能开关就是"交接方向盘"。两个方向 demo 都已自动处理：本原子自动开使能，motion03 检测到使能会自动关掉接管——来回切换零手动。
...
| 使能失败 / 自动关使能 | 别的程序在发 `/arm/cmd_pos`（如残留 motion03；也可能是遥控器 `teleop_dispatcher`） | `bash scripts/stop_all.sh` 清场后重来；`ros2 topic info /arm/cmd_pos -v` 查发布者，正常应只剩 1 个 |
...
- **末端（笛卡尔）控制**：本原子是**关节空间** MoveIt；想给"末端位姿"目标（而非关节角），见 **motion05（MoveIt 末端控制）**。
