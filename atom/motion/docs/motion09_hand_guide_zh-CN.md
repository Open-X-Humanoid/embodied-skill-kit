...
[English](motion09_hand_guide.md) | **简体中文**

**一句话**：向话题 `/inspire_hand/ctrl/left_hand` 发一条 `JointState`（6 个手指的张合百分比 0~1）让灵巧手张开/握拳，订阅 `/inspire_hand/state/left_hand` 读回当前张合。

| 配套代码 | 演示视频 |
|---|---|
| `atom/motion/motion09_hand_ros2.py`（ROS2 原生版）、`atom/motion/motion09_hand_ros2_robust.py`（生产版） | `atom/motion/assets/videos/` 下与配套代码同名 |

## 1. 速览（点进来先看这块）

### 1.1 跑起来

前提：已按《前置 · 环境配置》（`docs/environment_setup_zh-CN.md`）起好 body_control（灵巧手随本体驱动一起）。
...
`motion09_hand_ros2.py` 全文 = **5 个模块**。和头/臂是同一套「发指令 + 读状态」骨架，区别在**消息类型是 `JointState`、值是张合百分比**。
...
- **生产级加固**：等订阅就绪、状态超时拒动、显式校验（替代 `assert`，`-O` 下不失效）、值域夹到 `[0,1]`、状态长度不符告警、`spin_once` 非负 timeout——见 `atom/motion/motion09_hand_ros2_robust.py`。
