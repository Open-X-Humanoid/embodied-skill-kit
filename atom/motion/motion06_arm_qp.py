...
# -*- coding: utf-8 -*-
"""
原子示例 · 运控6 · 手臂（Arm）· XARM QP 关节空间控制
配套讲解：atom/motion/docs/motion06_arm_qp_guide.md

一句话
  把 7 个目标关节角交给 XARM 的 QP 关节控制器（jointspace_arm_L_controller），
  它在线做速度/加速度/jerk 平滑与限位检查后执行。
  和 motion04 的区别：motion04 是 MoveIt"一次规划整条轨迹再执行"；QP 是"响应式在线跟踪"，
  收到新目标立即平滑跟过去，适合目标不断变化的场景（如视觉伺服）。

控制器互斥（重要）
  moveit/jointspace/endpose 各族控制器都抢同一条臂的关节接口，同一时刻只能一个 active
  （例如刚跑过 motion07 的 endpose 控制器还 active，本控制器就激活不了）。
  本脚本激活前自动查询并停掉占用本臂的其它 active 控制器，再 STRICT 切换（失败如实报错）。

运行前提（x86 / ubuntu；比 motion04 少一步——QP 不需要 MoveIt 组件）
...
def main():
    ...
    node = ArmQpJointDemo()
    log = node.get_logger()
    log.info(f"运控6 · 手臂 QP 关节控制 | 控制器={QP_CONTROLLER} 演示增量={DEMO_DELTA}")
