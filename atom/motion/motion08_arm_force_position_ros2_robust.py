...
# -*- coding: utf-8 -*-
"""
原子示例 · 运控8 · 手臂力位混合模式（Force-Position Hybrid） —— ROS2 原生版 [生产级]
在关节空间演示力位混合接口：在位置目标基础上叠加速度/力矩前馈，
通过 kp/kd 决定关节"刚度"——小值柔顺（可被外力推开），大值刚硬。
配套讲解: atom/motion/docs/motion08_arm_force_position_guide.md
...
# 单次运动允许的最大位移量（安全上限，与 motion03_robust 的 MAX_DELTA_RAD 同一用途）
MAX_DELTA_RAD = 1.0  # rad，大于默认 CTRL_DELTA=0.5，仅用于兜底改错参数

# 关节角度软限位（rad），取自 tianyi2.0 URDF 硬限位、截断到两位小数，与
# motion03_arm_ros2_robust.py 的 JOINT_SOFT_LIMITS 保持一致（同一具身，共用限位表）。
...
class ArmForcePositionDemo(Node):
    ...
    def _validate_target(self, motor_id: int, current: float, target: float) -> bool:
        """
        ★ Bug修复5: 运动前做双重安全校验（与 motion03_arm_ros2_robust.py 同一套路）。
